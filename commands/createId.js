...
async function sendPrompt(message) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setTitle("<:Link:1521998525536272606>〃NOVɅ — CI citoyenne")
    .setDescription(
      "**Clique sur le bouton ci-dessous** pour remplir ta **carte d'identité**.\n" +
        "Afin de procéder à la **création de votre carte d'identité**, merci de **compléter les informations demandées** avec le plus grand soin. **Toutes les données fournies seront enregistrées dans nos registres** et **serviront à établir votre identité officielle.**"
    )
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      ...
      .setLabel("Crée votre CI")
      .setEmoji("<:Web:1522000825361235998>")
...
async function applyPostCreationEffects(interaction) {
  try {
    ...
    // Renomme automatiquement le membre en "[C]〃<nom d'affichage>" pour
    // marquer visuellement les citoyens enregistrés. On utilise le nom
    // d'affichage global du profil Discord (globalName), avec repli sur
    // le username si la personne n'en a pas défini.
    const prefix = "[C]〃";
    const maxNameLength = 32 - prefix.length;
    const displayName = (interaction.user.globalName ?? interaction.user.username).slice(
      0,
      maxNameLength
    );
    const newNickname = `${prefix}${displayName}`;
